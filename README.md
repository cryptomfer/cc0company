...
|| [`agentic-marketplace/`](./agentic-marketplace) | Pay-per-call services via x402 v2 (USDC on Base): AI image generation on 5 CC0 LoRAs, CC0 data services, and re-brokered mfergpt tools. Includes the canonical x402 client reference. |
|| [`autoshard/`](./autoshard) | Autonomous Base B20 token risk agent: scan any token for contract age, holder concentration, liquidity gaps, admin hooks, and generate a CC0 risk-card image through cc0.company image models. Headless JSON API. |
