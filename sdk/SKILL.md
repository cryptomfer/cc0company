...
| `launchToken({ name, symbol, image, feeTier, lpPreset?, pairedToken?, vault?, airdrop?, devBuyEth?, proceedsRecipient?, tokenAdmin?, … })` | 1 | self-paid fallback (and the only path on Ethereum / for dev buys). Registers automatically. `feeTier` is always `1` (fixed 1% static pool fee since 2026-09-09). `lpPreset` defaults `'degen'` (1.12.0+). Born-renounced: `tokenAdmin` defaults `address(0)`. `creatorRewards` slices are FEES ONLY — vault/airdrop admin + dev-buy proceeds go to the launching account or `proceedsRecipient` |
